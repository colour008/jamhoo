...
要了解`SSH key`简介，首先得熟悉`SSH`(Secure Shell的缩写 ) 是一个允许两台电脑之间通过安全的连接进行数据交换的网络协议。`SSH `密钥对可以让您方便的登录到 `SSH `服务器，而无需输入密码。`SSH `密钥对总是成双出现的，一把公钥，一把私钥。这里用到了非对称公钥加密体系，生成的公钥放到`github`的网站上，生成的私钥放在自己的电脑上。
